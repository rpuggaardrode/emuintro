...
* `.dft`: this contains a derived signal file of DFT (spectral) data obtained from the speech waveform; spectral processing is discussed in more detail in @sec-spectral_proc.
* `.fms`: this contains formant data also obtained from the speech waveform.
* `.wav`: this contains the waveform itself.

`.dft` and `.fms` are just examples of the sorts of signal files that can be stored in an Emu database -- this could also have been signals like pitch tracks or intensity, but for the advanced user, essentially any signal can be stored in an Emu database. We will see more examples in @sec-signalproc.
...
The `SSFF track definitions` show which signals are currently available in the database apart from the waveforms. For this database there are signals of type `dft` (discrete Fourier transform, see @sec-spectral_proc) and of type `fm` (formant). They have extensions `.dft` and `.fms`. This information is also given by:
...
It is not the purpose of this introduction to give explicit instruction on how to annotate which is covered amply in the [manual for the Emu Speech database management system](https://ips-lmu.github.io/The-EMU-SDMS-Manual), especially [section 9](https://ips-lmu.github.io/The-EMU-SDMS-Manual/chap-emu-webApp.html).
