# Converting a Praat TextGrid collection {#sec-praat2emu}
...
@fig-praat2emu exemplifies such a Praat `.TextGrid` collection and the `Emu-webApp` configuration we want to achieve:
...
The `summary()` above showed that there are no SSFF tracks associated with the database. Recall from @sec-first_steps that SSFF tracks store signal data that is timelocked to the audio signal. In this next step, we will generate SSFF tracks for `praat_emuDB` containing information about pitch.
...
The functions in `wrassp` are discussed in much more detail in @sec-signalproc.
...
`emupicture()` is under development, but it is a quite flexible function with functionality that goes way beyond what was shown here. See the [GitHub page](https://github.com/rpuggaardrode/praatpicture) for my information.
